...
| `signRank_reindex` | SignRank.lean | reindex invariance | sorry (routine) |
| `signRank_le_of_computableWithHeadsN` | SignRankBridge.lean | **028 bridge**: `H` heads ⇒ srank ≤ 2^(H+1)−2 | sorry (main own result; group cleared polynomial by head subsets, 2+2(2^H−2) rank-one pieces) |
| `signRank_le_pow_HStar` | SignRankBridge.lean | bridge at the optimum | **proved** modulo bridge |
| `signRank_le_of_thresholdDegLE` | SignRankBridge.lean | **Theorem C, degree half**: ceiling `(a+1)^d` | sorry (factor through monomials) |
...
| `warren_sign_patterns` | Warren.lean | **Warren 1968** | sorry (external target; hard: real algebraic geometry) |
| `forster` | Forster.lean | **Forster 2002** `N ≤ srank·‖M‖₂` | sorry (external target; hard: isotropic position argument) |
| `specNorm_kronecker`, `specNorm_reindex` | Forster.lean | spectral norm infrastructure | sorry (moderate linear algebra) |
| `thresholdDegLE_distThreshold` | DistanceThreshold.lean | `deg±(F_m) ≤ 2` | sorry (easy: `Δ − m/2` quadratic, m odd) |
| `thresholdDeg_distThreshold` | DistanceThreshold.lean | `deg±(F_m) = 2` | sorry (restriction to 2-bit XOR) |
| `specNorm_signMatrix_distThreshold` | DistanceThreshold.lean | `‖M‖₂ = 2·C(m−1,(m−1)/2)` | sorry (characters diagonalize; Fourier max at level 1) |
| `forsterRatio_le_signRank` | DistanceThreshold.lean | `γ_m ≤ srank(F_m)` | sorry (compose forster + specNorm value) |
| `theoremA` | DistanceThreshold.lean | **Theorem A** | sorry (compose) |
| `sqrt_le_forsterRatio` | DistanceThreshold.lean | `γ_m ≥ √(m−1)` | sorry (central binomial) |
| `four_le_HStar_distThreshold_127` | DistanceThreshold.lean | `H*(F_127) ≥ 4` | sorry (numeric: `14·C(126,63) < 2^126`) |
...
| `theoremB_gap` | Tensor.lean | **Theorem B** additive gap | sorry (logs from theoremB_HStar) |
...
| `HStar_ndisj_le` | NDISJ.lean | `H*(NDISJ_m) ≤ m` | sorry (corpus 029 monotone DNF) |
| `thresholdDeg_ndisj` | NDISJ.lean | `deg±(NDISJ_m) = 2` | sorry |
